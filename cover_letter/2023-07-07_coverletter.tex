...
We thank the reviewer for the positive assesment which encouraged us to further improve our manuscript. As a result, this revision is a major overhaul of the manuscript which followed the comments from the reviewers. Notably, we have extended the results to natural scenes which gives a broader impact to the paper. Given the substantial modifications we did to the original manuscript to reduce and clarify the different sections we do not provide a tracked changes version of the revision (such a document can be provided if needed). We provide below a point by point response to the reviewers with references to the manuscript.  
...
\end{quote} 
 ...
 \noteRev{By extending the learning and testing of our model on this newly defined motion detection task, we hope to have provided with a more thorough exploration of the capabilities of our model. The comparison of accuracy was also performed on the synthetic textures dataset in order to provide with a direct comparison with biological observations.}
