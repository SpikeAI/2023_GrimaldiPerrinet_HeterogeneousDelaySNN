...
% >>>>>>>>>>> LuP is here <<<<<<<<<<<
Lors de la définition de notre tâche, nous avons montré comment la génération d'événements dépendait du gradient dans les images. Ce gradient est défini dans les dimensions horizontale et verticale et les maxima de ce gradient sont généralement orientés. Considérés ensemble, ces bords orientés forment les contours des objets visuels présents dans la scène (Koenderink). Il existe donc une dépendance au sein du flux d'événements entre les informations relatives au mouvement et à l'orientation. Il serait crucial d'étudier plus en profondeur cette interdépendance, ce qui pourrait être amorcé en entraînant le modèle sur un ensemble de données comportant des labels d'orientation. L'étude de cette dépendance permettra de dissocier ces deux formes d'information visuelle et de pouvoir les intégrer, et notamment de considérer que la définition du mouvement est plus précise perpendiculairement à un contour orienté (aka, le problème de l'ouverture), mais aussi d'implémenter des règles prédictives récurrentes, telles que celles qui sont identifiées pour dissocier ce problème (Perrinet 2012).

Le modèle est entraîné sur une tâche de bas niveau de détection de mouvement local, et on peut se demander s'il pourrait être entraîné sur des tâches de plus haut niveau. Un exemple de tâche serait par exemple celui de l'estimation de la profondeur dans la scène visuelle. Il existe de multiples sources d'information pour déduire la profondeur, comme la disparité binoculaire ou les changements de texture ou d'ombrage, mais dans notre cas, la parallaxe du mouvement serait l'indice le plus significatif (Rogers). En effet, les objets proches d'un observateur se déplacent relativement plus rapidement sur la rétine qu'un objet situé à une grande distance. En utilisant cette information, on peut segmenter les objets et estimer leur profondeur (Yoonessi). Cependant, cela nécessiterait d'abord de calculer le flux optique. Une implémentation possible est donc d'ajouter une nouvelle couche à notre modèle, en analogie avec l'organisation hiérarchique mise en évidence dans le cortex visuel. Cela est en théorie possible en utilisant la sortie de notre modèle (qui estime la vitesse dans l'espace rétinotopique) comme entrée d'une nouvelle couche de neurones qui estimerait la vitesse dans le champ visuel, en incluant la dimension de profondeur comme étiquette de sortie. Cela pourrait avoir des applications directes et importantes, par exemple dans la conduite autonome, afin de détecter les obstacles de manière rapide et robuste. Une autre extension serait de produire activement des mouvements du capteur (physiquement ou virtuellement) pour produire cette estimation (Nawrot).

Pour conclure, le modèle que nous avons présenté offre un moyen de traiter efficacement les signaux basés sur des événements. Nous avons montré que nous pouvons entraîner le modèle de manière semi-supervisée, en connaissant les étiquettes de sortie mais sans savoir quand elles se produisent. Une autre perspective consisterait à étendre le modèle à un paradigme d'apprentissage entièrement auto-supervisé, c'est-à-dire sans aucune donnée étiquetée (Barlow). On pense que ce type d'apprentissage est prédominant dans le système nerveux central et, en supposant que le signal est parcimonieux (Olshausen), on pourrait étendre les schémas d'apprentissage hebbien parcimonieux aux spikes (Perrinet Masquelier). Un tel schéma serait particulièrement bien adapté à l'exploration de données neurobiologiques. En effet, il existe une littérature importante indiquant que la dynamique cérébrale s'organise souvent en séquences stéréotypées comme des chaînes synfire (Ikegaya), des paquets (Luczak) ou des séquences hippocampiques (Pastalkora 2008 Villette 2015). Ces motifs sont stéréotypés et robustes, car ils peuvent être activés selon le même motif d'un jour à l'autre (Haimerl 2019). Contrairement aux méthodes classiques qui sont utilisées pour traiter les données neurobiologiques, un tel modèle serait capable de répondre à des questions clés concernant la représentation de l'information dans les données neuronales. En outre, il ouvrirait des possibilités dans le domaine de l'apprentissage automatique, en particulier dans celui de la vision par ordinateur, pour résoudre les principales incertitudes actuelles telles que la robustesse aux attaques, la scalabilité, l'interprétabilité ou la consommation d'énergie.
