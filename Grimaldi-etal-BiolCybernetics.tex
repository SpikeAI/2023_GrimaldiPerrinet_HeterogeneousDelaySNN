...
%
%
Let's define formally the model, the task and how we will solve it. First, we will define the hetero-synaptic model which allows to learn in a  supervised manner the weights and delays of a feed-forward layer of spiking neural network (SNN). Our SNN model will be tested on an appropriate computational task of detecting motion in an event-based sensory input. By analogy with biological requirements, we will design a paradigm simulating eye movements where an input image is moved by a parameterized saccadic trajectory. The task is then to guess the movement as accurately as possible (see \seeFig{motion-task}). This is, for example, useful in real life situations to compensate for eye, head or body motions and provide a robust image categorization by stabilizing the image on the retina. For this, we will extend the SNN model to be efficiently adapted to solve the task by including convolutional kernels and describe its implementation on conventional computers. 
%
...
Once these eye movement trajectories are generated, we can apply them to a visual scene. For this purpose, we selected a database of 10 natural grayscale images that are commonly used to study the statistics of natural images [olshausen 96]. Note that these are pre-processed to equalize the energy in each frequency band (i.e., whitened). This process is known to occur as early as the retino-thalamic pathway [dan 1996]. These images are 512 × 512 in size and we will extract sub-images of size 128 x 128 which will be positioned around the center of the gaze at each time step (see \seeFig{motion-task}-(b)). We will discretize the time in 1ms bins and produce movies of duration N-T = 250 ms (i.e. approximately corresponding to the average duration between two saccades). To avoid border effects, we will position a draw of the complete trajectory at random in the image space so that the sub-image is translated using the position given by the trajectory at each time step. The translation is computed using a coordinate roll in the horizontal and vertical dimensions, followed by a sub-pixel translation defined in Fourier space [Perrinet, 2015]. Note that the magnitude of the displacement is relative to the time bin, and we have defined the velocity such that a velocity of V=1 corresponds to a movement of one pixel per frame (i.e., per time bin).
