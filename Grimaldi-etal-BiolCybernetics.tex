...
Let us first define a trajectory inspired by the biological movements of the eyes. Indeed, these movements allow us to dynamically actuate the center of vision in the field of vision. For example, saccades are rapid movements of the eyeball that reposition the center of vision towards points of interest. In humans, these are very frequent (on average every 3 seconds [ ]). They are produced very rapidly (about 80ms) and the maximum speed is proportional to the amplitude of the displacement (up to 600 deg/sec for a large saccade of 30deg amplitude). At a more microscopic scale, the gaze moves with an incessant drift similar to a Brownian-like trajectory (Poletti 2015). To maintain the full generality of the task, we defined eye movements using a form of random walk (Engbert 211). This approach first defines a defined set of possible 2D motions in polar coordinates. Based on the distribution of biological motions, we simplified it by selecting a set of eye movements as the Cartesian product of N-V-phi=8 linearly spaced motion directions and N-Y-velocity=7 different velocities (see inset of Fig: motion-task, a). Note that the velocities are sampled on a geometric scale between $1/4$ and $v-speed_base = 4$. Next, we define the trajectory as segments whose duration is drawn from a Poisson distribution with an average block length = 20 ms - similar to a Lévy flight. Finally, the trajectory is integrated by assuming 1/ that velocities are uniformly and independently sampled from the set of different motion-sets and 2/ that motion is uniform during a time segment. The resulting instances yield trajectories qualitatively similar to those observed for eye movements as observed for example in humans (see figure fig: motion-task - a).
...
Une fois que ces trajectoires de mouvements oculaires sont générées, nous pouvons les appliquer à une scène visuelle. Pour cela, nous avons sélectionné une base de données de 10 images naturelles en niveaux de gris qui sont typiquement utilisées pour étudier les statistiques des images naturelles [olshausen 96]. Notons que celles-ci sont prétraitées afin d'égaliser l'énergie dans chaque bande de fréquence (c'est-à-dire blanchies). Ce processus est connu pour se manifester dès la voie rétino-thalamique [dan 1996]. Ces images sont de taille 512 × 512 et nous allons extraire des sous-images de taille 128 x 128 qui seront positionnées autour du centre du regard à chaque pas de temps (voir figure fig : motion -task - b). Nous discrétiserons le temps en bins de 1ms réels et produirons des films de durée N-T = 250 ms (c'est-à-dire correspondant approximativement à la durée moyenne entre deux saccades). Pour éviter les effets de bord, nous positionnerons un tirage de la trajectoire complète au hasard dans l'espace image de telle sorte que la sous-image soit déplacée en utilisant la position donnée par la trajectoire à chaque pas de temps. La translation est calculée en utilisant un roulement de coordonnées dans les dimensions horizontale et verticale, suivi d'une translation sous-pixel définie dans l'espace de Fourier [Perrinet, 2015]. Notez que l'amplitude du déplacement est relative au bin de temps et nous avons défini la vitesse de telle sorte qu'une vitesse de V=1 correspond à un mouvement d'un pixel par image (c'est-à-dire par bin de temps).
