...
%
Our SNN model will be tested on a behaviorally relevant task consisting in detecting the motion within an event - based sensory input. Taking analogy with biological constraints We Will design a paradigm simulating eye movements where an input image is displaced by a parameterized jittered trajectory. The task is then to guess at best the motion (see fig : motion - task). This is, for instance useful in real-life situations to compensate for this motion and provide with motion-invariant image categorization by stabilizing the image on the retina.
...
Let's first define a biologically inspired trajectory inspired by eye movements. Indeed, these allow to dynamically actuate the axis of vision in the visual field of view. For instance, Saccades are rapid movements of the eyeball which reposition the center of vision. In humans, these are Very frequent (on average every 3 seconds [ ]). They are produced very rapidly C- 80ms) and the peak speed is proportional to the amplitude of the displacement (up to 600 deg/sec for a large saccade of 30deg of amplitude). At a more microscopic scale, the gaze moves with are incessant drift similar to a Brownian-like trajectory (Poletti 2015). To keep with the generality of the task, we defined eye movements using a form of Random walk (Engbert 211). This consists first to define a finite set of
possible 2D motions in polar coordinates. Based on the distribution of biological motions, we simplified it by selecting a set motion set of eye motions as the cartesian product of N-V-phi=8 linearly spaced motion directions and N-Y-speed=7 different speeds (see inset of fig : motion - task, a). Note that speeds are sampled on a geometric scale between $1/4$ and $v-speed_base = 4$. Second, we define the trajectory as segments whose duration are drawn from a Poisson distribution with an average length of block-length = 20 ms - similar to a Levy flight. Finally, the trajectory is integrated by assuming 1/that speeds are uniformly and independently sampled from the set of different motions motion-set and 2/ that motion is uniform during a time segment. resulting instances are giving qualitatively similar trajectories as those observed for eye movements as observed for instance in humans (see figure fig : motion-task - a) .
