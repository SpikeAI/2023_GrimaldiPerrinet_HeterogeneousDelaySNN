...
%%%-----------------------------------------------------------------
%
\subsection{Task definition: fast motion detection}\label{sec:task}
%%%-----------------------------------------------------------------
...
The goal is to test the model in a motion detection task using a realistic data stream. This type of signal is typically recorded by a Dynamic Vision Sensor (DVS). It is a good representation of the information and is inspired by the information sent by retinal ganglion cells through the optic nerve. Binary events allow the visual system to capture the dynamics of the natural scene and carry out different cognitive tasks quickly and effectively. To train and test our model, we need to define a dataset for which we control the direction and speed of motion.  First of all, let's define a trajectory that is inspired by the biological movements of the eyes. Indeed, these movements allow us to dynamically actuate the centre of vision, or gaze, in the visual field. In animals with a fovea, this is particularly useful as it allows the area with the highest density of photoreceptors in the environment to be moved, for example to a point of interest. A specific mechanism for this is called saccades, which are rapid eye movements that reposition the centre of vision. In humans, saccades are very frequent (on average $2 per second~\citep{dandekar_neural_2012}). They are generated very quickly (about $80~\ms$) and they have a whole range of speeds. On a more microscopic scale, the human gaze moves with a continuous drift similar to a Brownian trajectory~\citep{poletti_head-eye_2015}. In order to maintain the full generality of the task, we will define eye movements using a form of random walk~\citep{engbert_integrated_2011}. 
This approach first defines a finite set of possible 2D movements in polar coordinates. Based on the distribution of biological motions, we simplified it by selecting a set of eye movements as the Cartesian product of $8$ linearly spaced motion directions. Next, we define a gaze trajectory as segments whose duration is drawn from a Poisson distribution with a mean block length of $20~\ms$, similar to a Lévy flight~\citep[p. 289]{mandelbrot_fractal_1982}. Finally, the trajectory is integrated, assuming first that the velocities are sampled uniformly and independently from the set of different motion sets, and second that the motion is uniform during a time segment. The resulting instances yield trajectories that are qualitatively similar to those observed for human eye movements (\seeFig{motion_task}-(left)).
%%%-----------------------------------------------------------------
%: fig:motion_task
%%%-----------------------------------------------------------------
\begin{figure}%[h!]%[ht!]
    ...
    \caption{
    {\bf - Motion detection task.} {\bf (left)}~We use large natural images ($512\times512$) in which an aperture ($128\times128$) extracts a cropped  image around the axis of view. To mimic the effect of a saccadic eye movement, the view axis moves according to a stepwise random walk, and we show an example path of the trajectory as a green line ($200$ time steps). {\bf (right)}~Snapshot of the synthetic event stream at different time steps. The dynamics of the sub-image as a function of time generates a naturalistic film. This can be transformed into an event-based representation. Mimicking the retina, this representation encodes proportional and pixel-wise increases or decreases in luminance in the image, respectively ON (red) and OFF (blue) events. In the lower left corner of the images, the translation vector is shown in red. It is one of the possible classes of motion. Note the change in the direction of motion between the second and the third image.}
    \label{fig:motion_task}
\end{figure}
%%%-----------------------------------------------------------------
Once these eye trajectories are generated, we can apply them to a visual scene. For this purpose, we selected a database of 10 natural grayscale images that are commonly used to study the statistics of natural images~\citep{olshausen_emergence_1996}. Note that these have been pre-processed to equalize (i.e., whiten) the energy in each frequency band. This process is known to occur in the retino-thalamic pathway~\citep{dan_efficient_1996}. These images are $512 \times 512$ in size, and we will extract sub-images of size $128 \times 128$ positioned around the center of gaze at each time step. We will discretize time in $1~\ms$ bins and produce movies of duration $N_T = 250~\ms$. To avoid boundary effects, we will randomly position a plot of the full trajectory in image space so that the subimage is translated using the position given by the trajectory at each time step. The translation is computed using a coordinate roll in the horizontal and vertical dimensions, followed by a subpixel translation defined in Fourier space~\citep{perrinet_sparse_2015}. Note that the magnitude of the displacement is relative to the time bin, and we have defined the velocity to correspond to a movement of one pixel per frame (i.e., per time bin).
...
%%%-----------------------------------------------------------------
\subsubsection{The heterogeneous synaptic delay model}
