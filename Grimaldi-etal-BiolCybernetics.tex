...
\begin{figure}%[h!]
    ...
    \caption{Accuracy as a function of the number of computation load for the HD-SNN model (blue dots) with errorbars giving the 5\% - 95\% quantiles  and a sigmoid fit (blue line). The relative computational load (on a log axis) is controlled by changing the percentage of nonzero weights relative to the dense convolution kernel. As we prune coefficients, we observe a stable accuracy value, with a drop observed with approximately 25 times less computations.}
    \label{fig:accuracy}
\end{figure}
%
\section{Discussion}
