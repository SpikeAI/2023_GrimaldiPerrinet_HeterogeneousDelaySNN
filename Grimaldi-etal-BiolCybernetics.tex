...
\begin{figure}%[h!]%[ht!]
    ...
    \caption{
        {\bf - Motion Detection Task.} \textit{(Left)}~To generate realistic dynamic scenes, we mimic the effect of minute saccadic eye movements (with a length of $200~\ms$, green line) over a large natural scene ($1024\times1024$). For this, we crop an image ($128\times128$, for which we show luminance contrasts at time step $20~\ms$). The center of this aperture is moved dynamically according to a jagged random walk. \textit{(Right)}~The dynamics of the cropped image translated according to the saccadic path as a function of time produce a naturalistic movie, which is then transformed into an event-based representation. We show snapshots of the resulting synthetic event stream at different time steps (start and end frames marked by a white and black dot, respectively). Mimicking the response of ganglion cells in the retina, this representation encodes at each pixel all-or-none increases or decreases in luminance, i.e. ON (red) and OFF (blue) spikes. In the lower left corner of the snapshots, we show the corresponding instantaneous motion vector (red arrow). Note the change in the direction of motion between the third and fourth image, and also that contours parallel to the motion generate fewer luminance changes, the so-called aperture problem, and thus produce relatively fewer spikes.
        }
