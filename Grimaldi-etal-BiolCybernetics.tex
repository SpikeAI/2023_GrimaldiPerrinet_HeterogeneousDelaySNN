...
Pour synthétiser, nous avons introduit dans cet article un modèle de réseau de neurones à spikes avec des retards hétérosynaptiques que nous avons entraîné et évalué sur une tâche complexe de détection de mouvement. Le modèle a été défini comme devant détecter de manière optimale des motifs spatio-temporels événementiels. Nous avons montré qu'en l'entraînant sur un dataset d'images naturelles avec des mouvements oculaires réalistes, le modèle apprend des kernels similaires à ceux trouvés dans le cortex visuel précoce, des humains par exemple. Nous avons ensuite montré que la réponse du modèle a des réponses similaires à celles trouvées en biologie. Nous avons ensuite évalué le coût de calcul de ce modèle s'il était implémenté sur du matériel neuromorphique, montrant que le recours à des délais hétérosynaptiques peut s'avérer une solution de calcul frugale pour le futur hardware neuromorphique, mais aussi une clé pour comprendre pourquoi les spikes sont une composante universelle du traitement de l'information neuronale.

Soulignons quelques innovations dans les contributions présentées dans cet article. Tout d'abord, le modèle hétérosynaptique générique est formalisé à partir des premiers principes de détection optimale du signal spatio-temporel basé sur les événements, alors que [] utilise une heuristique basée sur la corrélation, dont nous avons prouvé qu'elle était moins efficace. De plus, par rapport à HOTS~\citep{lagorce_hots_2017} les poids sont explicables car ils donnent directement le logit (sigmoïde inverse de la probabilité) pour contribuer à détecter chaque motif de spiking spatio-temporel. Une autre nouveauté est que le modèle apprend simultanément les poids et les délais, alors que par exemple le modèle de polychronisation~\citep{Izhikevich 2006} n'apprend que les poids en utilisant la STDP alors que les délais sont tirés aléatoirement et leurs valeurs gelées. De plus, le modèle est évalué sur une tâche réaliste, alors que des modèles tels que le tempotron sont testés sur des problèmes simplifiés~\citep{gutig_tempotron_2006}. La contribution principale est d'apporter un modèle qui est adapté à n'importe quel type de motifs de spiking spatio-temporels et qui peut être entraîné de manière supervisée en fournissant un dataset de paires de supervision. Cela permet une définition plus flexible du modèle en utilisant ce dataset correctement étiqueté au lieu de s'appuyer sur une description minutieuse des règles physiques régissant une tâche, par exemple le principe de conservation de la luminance pour la détection de mouvement~\citep{benosman_asynchronous_2012, dardelet}.
...
We ientified several limits to our model,
the major of which we will now detail. First,
the full framework is based on discrete time which
is incompatible with the analogous nature of biological
time. We have used the binning of time to ease
the implementation of the computational framework
on conventional hardware, especially GPUs,
and in particular by using the three-dimensional
convolution. We have tested the effect of this time
bin size and have shown it does not impact
the results shown here. This is consistent with the
relative robustness of other ff event-based
frameworks such as thots, where accuracy was
not changed when jittering input spikes by
up to 1ms noise (grimaldi zz]. This hints
at including analytically a precision term in
the temporal value of incoming spikes, a
mechanism potentially implemented by the
n 5 ms smoothing at the level of the PSC
of conventional synapses. Furthermore, it is
possible to overcome the problem using a purely
even d- based scheme. Indeed, it is not necessary to
compute voltage males between two spikes (Morrison)
and to define a purely event-driven framework.
A further limit is that the model that
we defined is purely feed-forward. It
results that the spikes generated by post-synaptic
neurons are produced solely based on the
information in the classical receptive field.
However, it is known that neurons in a
layer may interact using lateral interaction
[charane Lorry and that this may be the
basis of computational principles, for instance
of binding neighboring orientations (Bednar).
Moreover, neural information is modulated by
feedback information, for instance do distinguish
figure from background (Roefsema J and
has proven to be essential in building realistic
models of the primary visual areas (Boutin 2020
Boutin rozo feedback , Boutin '2oz]. It is nd
currently possible to implant these recurrent
connections (lateral or feedback), mainly because
of our use of convolutions. However the generic
model is capable of including this by
inserting spikes in the list of spikes reaching
the pipe-synaptic addresses. While possible
in theory, it should be properly adapted
in practice such that these recurrent
connections do not reach outside an
homeostatic state ( a death or a
blow-up of activity).
Suchreccarrentatvityweential
to implement predictive or anticipative
processes . This is essential in a neural
system as it contains multiple different
delays which necessitates a temporal
alignment (Hoogendoorn 2019]. This was
previously modeled to explain for instance
the flash - lag visual illusion (Khoei 2017)
As was previously started " ... " (reminet
20174], and our hetero-synaptic model
would provide with a simple and elegant
