...
Nous avons identifié plusieurs limites à notre modèle, dont nous détaillons à présent les principales. Tout d'abord, le cadre complet est basé sur un binning discret du temps qui est incompatible avec la nature analogue du temps biologique. Nous avons utilisé ce binning pour faciliter l'implémentation du cadre de calcul sur du matériel conventionnel, notamment des GPU, et notamment pour être en mesure d'utiliser des convolutions tridimensionnelles. Nous avons testé l'effet de la taille du bin de temps et nous avons montré qu'il n'a pas d'impact sur les résultats présentés dans cet article. Cela est cohérent avec la robustesse relative d'autres cadres basés sur des événements tels que HOTS (Lagorce 2017), où la précision n'a pas été modifiée lorsque les spikes d'entrée ont été soumis à un bruit allant jusqu'à 1ms (grimaldi 2022). Cela suggère la possibilité d'inclure analytiquement un terme de précision dans la valeur temporelle des spikes en entrée, un mécanisme potentiellement mis en œuvre par le filtrage d'environ 5 ms qui est mis en œuvre au niveau du PSC des synapses. De plus, il est possible de surmonter le problème en utilisant un schéma purement basé sur les événements. En effet, il n'est pas nécessaire de calculer les traces de voltage entre deux spikes (Morrison) et donc de définir un cadre purement événementiel. Une telle architecture pourrait fournir des gains de vitesse prometteurs pour les calculs.

Une autre limite est que le modèle que nous avons défini est purement feed-forward. Par conséquent, les spikes générés par les neurones post-synaptiques sont produits uniquement sur la base des informations contenues dans le champ réceptif classique. Cependant, il est reconnu que les neurones d'une couche peuvent interagir en utilisant des interactions latérales (chavane 2022) et que cela peut être la base de principes computationnels, par exemple de la combinaison d'orientations voisines qui peut être une composante du mécanisme de catégorisation des images (Bednar). De plus, l'information neuronale est modulée par l'information en feed-back, par exemple pour distinguer une figure de l'arrière-plan (Roelfsema 2016) et s'est avérée essentielle pour construire des modèles réalistes des aires visuelles primaires (Boutin 2020 Boutin 2020 feedback , Boutin 2022). Il n'est actuellement pas possible d'implémenter ces connexions récurrentes dans notre implémentation (latérale ou feedback), principalement en raison de notre utilisation des convolutions. Cependant le modèle théorique générique est capable de les inclure en insérant des spikes dans la liste des spikes qui atteignent les adresses pré-synaptiques. Bien que cela soit possible en théorie, il convient de l'adapter correctement en pratique afin que ces connexions récurrentes n'amplifient pas l'activité neuronale en dehors d'un état homéostatique (par une mort ou une explosion de l'activité). 

Une telle activité récurrente serait essentielle pour mettre en œuvre des processus prédictifs ou anticipatifs. Ceci est essentiel dans un système neuronal car il contient de multiples retards différents qui nécessitent un alignement temporel (Hoogendoorn 2019]. Ceci a été précédemment modélisé pour expliquer par exemple l'illusion de flash - lag (Khoei 2017) Comme cela a été précédemment commencé " .... " (Perrinet 2014], et notre modèle hétérosynaptique fournirait une solution simple et élégante à ce problème.
