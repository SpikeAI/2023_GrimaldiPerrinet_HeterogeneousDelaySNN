...
\subsection{Outline}
%%%-----------------------------------------------------------------
In this work, we study the emergence of such spatio-temporal spiking motifs when training a single layer of spiking neurons on a supervised classification task (\seeFig{motion_task}). The paper is organized as fellows. We develop a SNN-like method able to learn hetero-synaptic delays to perform motion detection on a synthetic event-based dataset.   We will first details the  méthodologie by first defining the  core spiking mechanism using  hetero-synaptic delays. We will  then define formally the computational  task of notion detection. This  will allow us to formalize the  final spiking neuron used to learn  the model and test its efficiency.  In the results section, we will  detail the emerging structure  of the Sun solving the task and  in particular show the similarities  with neurobiological observations. We  will also test the network with  classical stimuli used in neuroscience,  and notably bitrate the response  to frequencies or contrast.  We will further study the efficiency  of the motion detection mechanism  and in particular how it is  resilient to the pruning of synaptic  Weights. This will allow to show  how such model may provide an efficient solution to the energy/  accuracy trade-off.  Finally, we will conclude lay  highlighting major contributions  of this paper, while delineating  some limits. This will open up  to perspectives for future SNN.  Because neuromorphic devices are, by design, good candidates to integrate computations with time, we highlight the fact that this event-driven algorithm is transferable to such hardware.
%
\section{Methods}
\label{sec:methods}
%%%-----------------------------------------------------------------
