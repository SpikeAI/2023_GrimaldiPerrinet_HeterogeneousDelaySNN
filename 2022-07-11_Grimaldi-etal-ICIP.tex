...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% CONF: https://2022.ieeeicip.org/
% dates: https://2022.ieeeicip.org/important-dates/
%
% paper kit: https://cmsworkshops.com/ICIP2022/papers/paper_kit.php#Templates
% actual submission : https://cmsworkshops.com/icip2022/Papers/Submission.asp
%
% 4 pages + one of references
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%Review #0357
...
%This paper proposes a model for detection of temporal spiking motifs, which can potentially improve the performance of spiking neural networks (SNN). The topic is out of my research scope. However, I am not sure how this paper is related to ICIP. The authors applied the proposed model in motion detection. Such motion is captured by imaging or not? The overall presentation of the paper is acceptable, but I would suggest the authors to more explicitly represent the contribution of the proposed model and how it is expected to benefit image processing studies.
%
%-----------
%Review #1B91
%
%*General Comments to Authors*
%The authors propose a Spiking Neural Network-like method that consists of a single layer of spiking neurons aiming to learn the heterosexual-synaptic delays in order to detect motion. The learning happens in a supervised way as the authors transform the Motion Cloud dataset into spike trains in order to control the direction of motion which is the ground-truth information. The authors considered 8 different directions of motion that are assigned to the 8 learnt kernels they introduce in their model. According to the experimental results, the model is able to capture the direction of motion which is evident to the selectivity along the specific axes of the different kernels. Last but not least, The authors assess their model in terms of the classification accuracy as a function of the computational cost. They show that on the one hand, their method is able to highly reduce the computational load by thresholding some of the weights but at the same time achieve the same performance to computationally more greedy algorithms. The paper is very interesting and promising as this method can be easily extended to hardware.
%
%Some suggestions to improve the presentation of the results are given below:
%1. Figure 2 is a bit confusing. First of all, in (b) it's not that evident the different size of the radius but in few cases - maybe another example could be selected. In addition, if (a)-(b) and (c) is a single experiment how the delay is related to the time we observe the spike trains? Maybe that could be better explained in the text too.
%
%Last but not least, are these 8 directions enough to capture every motion? Could we use this model in real data or the computational cost would increase as one should consider much higher number of motion directions and as a results much larger set of kernels to be trained?
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newcommand{\Title}{
...
%\newcommand{\mycaption}[1]{\captionof{figure}{#1}}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
